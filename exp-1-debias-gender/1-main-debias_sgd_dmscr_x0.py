...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            loss_img_x0_l2_i = torch.ones(targets.shape, dtype=weight_dtype, device=accelerator.device) *(-1)
            ...
            for j in range(N_backward):
                ...
                images_ori_ij = images_ori[idxs_ij]
                ...
                loss_img_x0_l2_ij = ((images_ij - images_ori_ij) ** 2).mean(dim=(1, 2, 3)).to(weight_dtype)
                ...
                dynamic_weights = gen_dynamic_weights_sds(
                    ...
                    preds_gender_ij,
                    factor=args.factor1,
                    out_dtype=loss_img_x0_l2_ij.dtype,
                )

                loss_ij = (
                    loss_fair_ij
                    + args.weight_loss_img * dynamic_weights * loss_img_x0_l2_ij
                ...
                with torch.no_grad():
                    ...
                    loss_img_x0_l2_i[idxs_ij] = loss_img_x0_l2_ij.to(loss_img_x0_l2_i.dtype)
            ...
            loss_img_x0_l2_all = customized_all_gather(loss_img_x0_l2_i, accelerator)
            ...
            if accelerator.is_main_process:
                ...
                logs_i["loss_img_x0_l2"].append(loss_img_x0_l2_all)
                logs_i["loss"].append(loss_all)
            
            # process logs
            if accelerator.is_main_process:
                for key in ["loss_fair", "loss_face_realistic", "loss_img_x0_l2", "loss"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
                    else:
