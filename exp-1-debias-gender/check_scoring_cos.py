...
def main(args):
    ...
    def sds_logits_from_images(
        ...
        [check_scoring_cos] img-preservation loss (compute_img_hspace=True):
            Instead of comparing mid-block features along the *generation* trajectory, this
            measures image semantics preservation INSIDE the SDS forward. The finetune x0
            (`images`) and the original-DM x0 (`ref_images`) are noised with the SAME eps/t
            at each of the `num_t` SDS timesteps, run through `sds_unet` with the generation
            prompt `img_prompt`, and their mid_block bottlenecks are compared with cosine
            similarity. The returned img loss is the mean of (1 - cos) over the num_t (x num_eps)
            samples. Gradient flows only through the finetune branch; the reference is detached.
        """
        B = images.shape[0]
        ...
        # =========================
        # [check_scoring_cos] image-preservation loss via SDS mid-block cosine.
        #   At every SDS timestep, run sds_unet on BOTH the finetune x0 (zt) and the
        #   original-DM x0 (zt_ref) using the generation prompt (img_prompt). Capture each
        #   mid_block bottleneck and take 1 - cosine_similarity. Mean over the num_t (x num_eps)
        #   samples gives the per-image img loss. Gradient flows only through the finetune
        #   branch (zt -> images); the original-DM branch is fully detached.
        # =========================
        img_hspace_loss = None
        if compute_img_hspace:
            if ref_images is None or img_prompt is None:
                raise ValueError(
                    "compute_img_hspace=True requires both ref_images (original-DM x0) and img_prompt."
                )
            # Reference (original-DM x0) latents, noised with the SAME eps/t as the finetune
            # branch so the only difference fed to the UNet is the image content.
            with torch.no_grad():
                ref_lat = vae.encode(ref_images.to(weight_dtype)).latent_dist.sample() * vae.config.scaling_factor
                ref_lat_exp = (
                    ref_lat.unsqueeze(1)
                    .unsqueeze(2)
                    .expand(B, K, num_eps, *ref_lat.shape[1:])
                    .contiguous()
                    .view(BKE, *ref_lat.shape[1:])
                )
                zt_ref = (
                    alpha_bar.sqrt().view(-1, 1, 1, 1) * ref_lat_exp
                    + (1.0 - alpha_bar).sqrt().view(-1, 1, 1, 1) * eps
                )

            pe_img = _text_embeds([img_prompt] * BKE, sds_text_encoder).to(weight_dtype)

            mb_capture = MidBlockCapture().attach(sds_unet)
            try:
                # finetune branch: keeps gradient through zt -> lat -> images
                mb_capture.last = None
                _ = sds_unet(zt.to(weight_dtype), t_vec, encoder_hidden_states=pe_img).sample
                h_fine = mb_capture.last
                # original-DM branch: detached reference
                with torch.no_grad():
                    mb_capture.last = None
                    _ = sds_unet(zt_ref.to(weight_dtype), t_vec, encoder_hidden_states=pe_img).sample
                    h_ref = mb_capture.last
            finally:
                mb_capture.detach()

            if h_fine is None or h_ref is None:
                raise RuntimeError("Failed to capture mid_block h-space during SDS img loss.")

            # cosine distance in fp32; reduce to per-image [B] by mean over (K, num_eps).
            with torch.autocast("cuda", enabled=False):
                cos = F.cosine_similarity(
                    h_fine.float().flatten(1),
                    h_ref.detach().float().flatten(1),
                    dim=1,
                )  # [BKE]
                img_hspace_loss = (1.0 - cos).view(B, K, num_eps).mean(dim=(1, 2))  # [B], fp32
        ...
        if return_attmap:
            return preds, probs32, logits32, sds_f32, sds_m32, sds_r32, img_hspace_loss, attmap_mean
        return preds, probs32, logits32, sds_f32, sds_m32, sds_r32, img_hspace_loss
    ...
    def evaluate_process(which_text_encoder, which_unet, name, prompts, noises, current_global_step, enable_sds_eval: bool = True):
        ...
        for prompt_i, noises_i in itertools.zip_longest(prompts, noises):
            ...
            attmap_ori = None
            if need_sds_forward:
                preds_gender_ori_sds, probs_gender_ori_sds, logits_gender_ori_sds, _, _, _, _, attmap_ori = sds_logits_from_images(
                    images_ori,
                    tau=args.sds_tau,
            ...
            attmap_gen = None
            if need_sds_forward:
                preds_gender_sds, probs_gender_sds, logits_gender_sds, _, _, _, _, attmap_gen = sds_logits_from_images(
                    images,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                preds_gender_sds, probs_gender_sds, logits_gender_sds, _, _, _, _ = sds_logits_from_images(
                    images,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
            ...
            for j in range(N_backward):
                ...
                # Finetune x0 (keeps gradient). The img-preservation loss is now computed
                # INSIDE the SDS forward (mid-block cosine vs the original-DM x0), so we no
                # longer ask generate_image_w_gradient for the trajectory h-space loss.
                ...
                # Original-DM x0 for the SAME noises (generated under no_grad in Step 3,
                # same prompt / num_denoising_steps / skip config), used as the detached
                # reference for the SDS mid-block cosine img loss.
                ref_images_ij = images_ori[idxs_ij]

                # --- SDS classifier WITH gradient + region hard mask + img-preservation loss ---
                preds_gender_ij, probs_gender_ij, logits_gender_ij, _, _, sds_realistic_ij, loss_hspace_ij = sds_logits_from_images(
                    images_ij,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                    ...
                    ref_images=ref_images_ij,
                    img_prompt=prompt_i,
                    compute_img_hspace=True,
                )
