...
def main(args):
    ...
    def evaluate_process(which_text_encoder, which_unet, name, prompts, noises, current_global_step, enable_sds_eval: bool = True):
        ...
        if accelerator.is_main_process:
            ...
            if name == "EMA":
                metrics_to_save = {}
                # Save EVERY aggregated eval metric (all gender_gap variants, bias_score, ...)
                if logs:
                    for key in logs[0].keys():
                        try:
                            arr = np.array([log[key] for log in logs], dtype=float)
                        except (TypeError, ValueError):
                            continue
                        val = float(np.nanmean(arr))
                        if not np.isnan(val):
                            metrics_to_save[f"eval_EMA_{key}"] = val
                # Similarity metrics (computed separately, not in `logs`)
                if len(clip_t_sims) > 0:
