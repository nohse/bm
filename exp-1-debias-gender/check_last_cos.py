...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--h_loss_form", type=str, default="cos",
        choices=["cos", "mse", "raw"],
        help="h-space distance form during generation steps. "
             "'cos' = 1 - cosine_similarity on flattened h (default). "
             "'mse' = elementwise mean squared error. "
             "'raw' is accepted as a legacy alias for 'mse'.",
    )
    ...
    parser.add_argument(
        "--skip_final_steps_pct", type=float, default=0,
    ...
    args.h_loss_form = str(args.h_loss_form).strip().lower()
    if args.h_loss_form == "raw":
        args.h_loss_form = "mse"
    if args.h_loss_form not in {"cos", "mse"}:
        raise ValueError("h_loss_form must be one of: cos, mse")
...
def main(args):
    ...
    # the run-defining knobs -> region_mask_mode (attn/none), h-space loss form,
    # final-step skip %, and each loss weight (img / realistic-face). NOTE: the fair-loss fields (_wFair-/_fcw-)
    ...
    def generate_image_w_gradient(
        ...
        try:
            # [check_last_cos] The image-preservation (h-space) loss uses ONLY the LAST
            # executed denoising step's mid-block output. The distance form is selected by
            # --h_loss_form. The reference trajectory is still advanced at EVERY step
            # (under no_grad) so its mid-block output at the final step is the correct one;
            # intermediate steps contribute no loss term.
            # NOTE: "last step" = last step actually run (timesteps[steps_to_run - 1]); when
            # skip_final_steps(_pct) > 0 the skipped tail is not run, so this is the last
            # gradient-bearing step before the x0 jump.
            last_hspace_step = steps_to_run - 1
            for i, t in enumerate(timesteps[:steps_to_run]):
                ...
                if return_hspace_loss:
                    ...
                    # Only the final executed step contributes to the loss.
                    if i == last_hspace_step:
                        ...
                        form = getattr(args, "h_loss_form", "cos")
                        if form == "cos":
                            # Last-step h-space cosine distance (scale-invariant), per sample.
                            ...
                            raise ValueError(f"Unsupported h_loss_form: {form}")
