...
def main(args):
    ...
    def gen_dynamic_weights_sds(face_indicators, targets, preds_ori_sds, factor=0.2, out_dtype=None):
        ...
        reference와 똑같이 얼굴 검출 여부(face_indicators)와, target vs original-image의
        예측 class를 비교해 image-보존 loss의 per-sample 가중치를 정한다. 다만 class는 mnet이
        아니라 기존처럼 SDS로 잰 original 이미지 예측(preds_ori_sds)을 사용한다.
        - 얼굴 미검출 (face_indicator == False) : 1
        - target == -1                          : factor
        ...
        weights = []
        for face_indicator, target, pred_ori_sds in itertools.zip_longest(face_indicators, targets, preds_ori_sds):
            if (face_indicator == False).all():
                weights.append(1)
            else:
                if target == -1:
                    weights.append(factor)
                elif target == pred_ori_sds:
                    weights.append(1)
                elif target != pred_ori_sds:
                    weights.append(factor)
        weights = torch.tensor(weights, dtype=out_dtype, device=targets.device)
        return weights
