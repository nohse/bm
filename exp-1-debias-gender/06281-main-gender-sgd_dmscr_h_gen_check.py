...
from accelerate.utils import ProjectConfiguration, set_seed, GradScalerKwargs, InitProcessGroupKwargs
from transformers import CLIPModel, CLIPProcessor
# [ADDED] BLIP captioning model, used as an extra eval-time gender classifier
...
def parse_args(input_args=None):
    ...
    # =====================
    # [ADDED] BLIP captioning-based gender classifier (evaluation only)
    # =====================
    parser.add_argument(
        "--blip_model_name", type=str, default="Salesforce/blip-image-captioning-large",
        help="평가 시 captioning으로 성별을 분류할 BLIP 모델 이름",
    )
    parser.add_argument(
        "--blip_gender_prompt", type=str, default="the gender of the face is",
        help="BLIP conditional captioning에 넣을 프롬프트 (이 문장 뒤를 모델이 이어서 생성).",
    )
    parser.add_argument(
        "--blip_batch_size", type=int, default=16,
        help="BLIP gender captioning 시 배치 크기",
    )
    parser.add_argument(
        "--blip_max_new_tokens", type=int, default=20,
        help="BLIP gender caption 생성 시 최대 토큰 수",
    )
    parser.add_argument(
...
def main(args):
    ...
    # Raise the NCCL collective timeout from the 10-min default to 60 min. At startup
    # the main process (rank 0) alone downloads/loads the large eval-only models
    # (open_clip ViT-bigG-14, DINOv2 vit-g/14) and now the BLIP captioning model;
    # meanwhile the other ranks reach a collective and wait. Without this raised
    # timeout the waiting rank hits the 600s watchdog and SIGABRTs the whole run.
    # The slow FR (dlib CNN) face-detection fallback during eval has the same effect.
    ...
    folder_name = f"BS-{args.train_images_per_prompt_GPU*accelerator.num_processes}_wImg-{args.weight_loss_img}-{args.factor1}-{args.factor2}_wFace-{args.weight_loss_face}_Th-{args.uncertainty_threshold}_loraR-{args.rank}_lr-{args.learning_rate}_skip-{int(args.skip_final_steps_pct)}_{timestring}"
    ...
    # ------------------------------------------------------------------
    # [ADDED] BLIP captioning model as an additional eval-time gender classifier.
    # Loaded on every process (mirrors the MobileNet classifier) so each process
    # can caption its own face chips and the per-process results are all-gathered,
    # exactly like the mnet metrics.
    # ------------------------------------------------------------------
    blip_processor = BlipProcessor.from_pretrained(args.blip_model_name)
    blip_model = BlipForConditionalGeneration.from_pretrained(
        args.blip_model_name, torch_dtype=weight_dtype
    )
    blip_model.to(accelerator.device)
    blip_model.requires_grad_(False)
    blip_model.eval()
    if accelerator.is_main_process:
        logger.info(f"[blip] loaded captioning model from {args.blip_model_name}")
    ...
    # [ADDED] BLIP captioning-based gender classifier (evaluation only).
    # ------------------------------------------------------------------
    def _parse_gender_from_caption(caption: str):
        """Map a BLIP caption to a binary gender vote.

        Returns (pred, probs) where
            pred : 1 (man), 0 (woman), or -1 (undecided / no gender word)
            probs: [P(woman), P(man)] one-hot, or [-1,-1] if undecided.
        Word boundaries (\\b) stop 'female' from matching 'male' and
        'woman' from matching 'man'.
        """
        c = caption.lower()
        n_male = len(re.findall(
            r"\b(?:male|males|man|men|boy|boys|gentleman|gentlemen|guy|guys|he|him|his|masculine)\b",
            c,
        ))
        n_female = len(re.findall(
            r"\b(?:female|females|woman|women|girl|girls|lady|ladies|she|her|hers|feminine)\b",
            c,
        ))
        if n_male > n_female:
            return 1, [0.0, 1.0]
    ...
    @torch.no_grad()
    def get_face_gender_blip(face_chips, selector=None, fill_value=-1):
        """BLIP-captioning gender classifier (evaluation only).

        Mirrors get_face_gender(): only face-detected chips (selector==True) are
        captioned. Each chip is fed to BLIP with the conditional prompt
        `args.blip_gender_prompt` ("the gender of the face is") and the generated
        caption is parsed into a hard male/female vote. Returns preds/probs/logits
        with the SAME [N] / [N,2] layout as get_face_gender, so the downstream
        gender-gap math is identical (fill_value rows for chips without a detected
        face or without a decided gender word).
            preds : [N] int64   (1: man, 0: woman, fill_value: no face / undecided)
            probs : [N,2] float ([P(woman), P(man)], fill_value rows otherwise)
            logits: [N,2] float (== probs; kept only for signature parity)
        """
        ...
        if face_chips_w_faces.shape[0] == 0:
            probs_gender = torch.empty([0, 2], dtype=torch.float32, device=face_chips.device)
            preds_gender = torch.empty([0], dtype=torch.int64, device=face_chips.device)
        else:
            # [-1,1] -> [0,1] -> list of PIL face images for the BLIP processor
            imgs01 = (face_chips_w_faces.detach().float().cpu() * 0.5 + 0.5).clamp(0, 1)
            pil_faces = [T.ToPILImage()(img) for img in imgs01]

            preds_list = []
            probs_list = []
            bs = max(1, int(args.blip_batch_size))
            for k in range(0, len(pil_faces), bs):
                batch_imgs = pil_faces[k:k + bs]
                inputs = blip_processor(
                    images=batch_imgs,
                    text=[args.blip_gender_prompt] * len(batch_imgs),
                    return_tensors="pt",
                )
                inputs = {kk: vv.to(face_chips.device) for kk, vv in inputs.items()}
                if "pixel_values" in inputs:
                    inputs["pixel_values"] = inputs["pixel_values"].to(weight_dtype)
                generated_ids = blip_model.generate(
                    **inputs,
                    max_new_tokens=int(args.blip_max_new_tokens),
                    num_beams=1,
                )
                captions = blip_processor.batch_decode(generated_ids, skip_special_tokens=True)
                for cap in captions:
                    pred, prob = _parse_gender_from_caption(cap)
                    preds_list.append(pred)
                    probs_list.append(prob)

            preds_gender = torch.tensor(preds_list, dtype=torch.int64, device=face_chips.device)
            probs_gender = torch.tensor(probs_list, dtype=torch.float32, device=face_chips.device)

        if selector is not None:
            preds_gender_new = torch.ones(
                [selector.shape[0]], dtype=torch.int64, device=face_chips.device
            ) * int(fill_value)
            preds_gender_new[selector] = preds_gender

            probs_gender_new = torch.ones(
                [selector.shape[0], 2], dtype=torch.float32, device=face_chips.device
            ) * float(fill_value)
            probs_gender_new[selector] = probs_gender

            logits_gender_new = probs_gender_new.clone()
            return preds_gender_new, probs_gender_new, logits_gender_new
        else:
            return preds_gender, probs_gender, probs_gender.clone()
    ...
    def evaluate_process(which_text_encoder, which_unet, name, prompts, noises, current_global_step, enable_sds_eval: bool = True):
        ...
        for prompt_i, noises_i in itertools.zip_longest(prompts, noises):
            ...
            # --- BLIP captioning classifier (evaluation only; face-detected chips only) ---
            preds_gender_ori_blip, probs_gender_ori_blip, _ = get_face_gender_blip(
                face_chips_ori,
                selector=face_indicators_ori,
                fill_value=-1,
            )
            ...
            preds_gender_ori_all_blip = customized_all_gather(preds_gender_ori_blip, accelerator, return_tensor_other_processes=False)
            probs_gender_ori_all_blip = customized_all_gather(probs_gender_ori_blip, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                save_to_blip = os.path.join(args.imgs_save_dir, f"eval_{name}_{global_step}_{prompt_i}_ori_blip.jpg")
                plot_in_grid(
                    images_ori_all,
                    save_to_blip,
                    face_indicators=face_indicators_ori_all,
                    face_bboxs=face_bboxs_ori_all,
                    preds_gender=preds_gender_ori_all_blip,
                    pred_class_probs_gender=probs_gender_ori_all_blip.max(dim=-1).values,
                )
            ...
            # --- BLIP captioning classifier (evaluation only; face-detected chips only) ---
            preds_gender_blip, probs_gender_blip, _ = get_face_gender_blip(
                face_chips,
                selector=face_indicators,
                fill_value=-1,
            )
            ...
            preds_gender_all_blip = customized_all_gather(preds_gender_blip, accelerator, return_tensor_other_processes=False)
            probs_gender_all_blip = customized_all_gather(probs_gender_blip, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                save_to_blip = os.path.join(args.imgs_save_dir, f"eval_{name}_{global_step}_{prompt_i}_generated_blip.jpg")
                plot_in_grid(
                    images_all,
                    save_to_blip,
                    face_indicators=face_indicators_all,
                    face_bboxs=face_bboxs_all,
                    preds_gender=preds_gender_all_blip,
                    pred_class_probs_gender=probs_gender_all_blip.max(dim=-1).values,
                )
            ...
            if accelerator.is_main_process:
                ...
                # --- BLIP captioning-based gender gap (generated + ori) ---
                probs_tmp_blip = probs_gender_all_blip[(probs_gender_all_blip!=-1).all(dim=-1)]
                if probs_tmp_blip.shape[0] > 0:
                    gender_gap_blip = (
                        ((probs_tmp_blip[:,1]>=0.5)*(probs_tmp_blip[:,1]<=1)).float().mean()
                        - ((probs_tmp_blip[:,1]>=0)*(probs_tmp_blip[:,1]<=0.5)).float().mean()
                    ).item()
                    logs_i["gender_gap_blip"].append(gender_gap_blip)
                    logs_i["gender_gap_abs_blip"].append(abs(gender_gap_blip))
                else:
                    logs_i["gender_gap_blip"].append(np.nan)
                    logs_i["gender_gap_abs_blip"].append(np.nan)
                probs_tmp_blip_ori = probs_gender_ori_all_blip[(probs_gender_ori_all_blip!=-1).all(dim=-1)]
                if probs_tmp_blip_ori.shape[0] > 0:
                    gender_gap_blip_ori = (
                        ((probs_tmp_blip_ori[:,1]>=0.5)*(probs_tmp_blip_ori[:,1]<=1)).float().mean()
                        - ((probs_tmp_blip_ori[:,1]>=0)*(probs_tmp_blip_ori[:,1]<=0.5)).float().mean()
                    ).item()
                    logs_i["gender_gap_abs_blip_ori"].append(abs(gender_gap_blip_ori))
                else:
                    logs_i["gender_gap_abs_blip_ori"].append(np.nan)
        ...
        if accelerator.is_main_process:
            ...
            if name == "EMA":
                ...
                if logs and "gender_gap_abs_blip" in logs[0]:
                    gg_blip = np.array([log["gender_gap_abs_blip"] for log in logs], dtype=float)
                    metrics_to_save["eval_EMA_gender_gap_abs_blip"] = float(np.nanmean(gg_blip))
                if len(clip_t_sims) > 0:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            # Original DM (anchor) used for per-step h-space matching during generation.
            if args.train_text_encoder and args.train_unet:
                h_reference_text_encoder = eval_text_encoder
                h_reference_unet = eval_unet
            elif args.train_text_encoder and not args.train_unet:
                h_reference_text_encoder = eval_text_encoder
                h_reference_unet = unet
            else:
                h_reference_text_encoder = text_encoder
                h_reference_unet = eval_unet
            ...
            for j in range(N_backward):
                ...
                images_ij, _att_ij, loss_hspace_ij = generate_image_w_gradient(
                    prompt_i,
                    noises_ij,
                    num_denoising_steps,
                    which_text_encoder=text_encoder,
                    ...
                    h_reference_text_encoder=h_reference_text_encoder,
                    h_reference_unet=h_reference_unet,
                )
