...
def main(args):
    ...
    def sds_logits_from_images(
        ...
        [check_scoring_mse] img-preservation loss (compute_img_hspace=True):
            ...
            prompt `img_prompt`, and their mid_block bottlenecks are compared with elementwise
            MSE. The returned img loss is the mean squared error over the num_t (x num_eps)
        ...
        # =========================
        # [check_scoring_mse] image-preservation loss via SDS mid-block MSE.
        ...
        #   mid_block bottleneck and take elementwise MSE. Mean over the num_t (x num_eps)
        ...
        if compute_img_hspace:
            ...
            # elementwise MSE in fp32; reduce to per-image [B] by mean over (K, num_eps).
            with torch.autocast("cuda", enabled=False):
                mse = (
                    (h_fine.float().flatten(1) - h_ref.detach().float().flatten(1)) ** 2
                ).mean(dim=1)  # [BKE]
                img_hspace_loss = mse.view(B, K, num_eps).mean(dim=(1, 2))  # [B], fp32
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            for j in range(N_backward):
                ...
                # INSIDE the SDS forward (mid-block MSE vs the original-DM x0), so we no
                ...
                # reference for the SDS mid-block MSE img loss.
