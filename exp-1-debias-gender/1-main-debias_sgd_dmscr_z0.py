        ...
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                for j in range(N):
                    noises_ij = noises_i[args.val_GPU_batch_size*j:args.val_GPU_batch_size*(j+1)]
                    if args.train_text_encoder and args.train_unet:
                        images_ij, _att_ij, z0_ij = generate_image_no_gradient(
                            prompt_i,
                            noises_ij,
                            num_denoising_steps,
            ...
            loss_img_z0_l2_i = torch.ones(targets.shape, dtype=weight_dtype, device=accelerator.device) *(-1)
            ...
            for j in range(N_backward):
                ...
                images_ij, _att_ij, z0_finetune_ij = generate_image_w_gradient(
                    prompt_i,
                    noises_ij,
                    num_denoising_steps,
                    which_text_encoder=text_encoder,
                ...
                loss_img_z0_l2_ij = ((z0_finetune_ij - z0_ori_ij) ** 2).mean(dim=(1, 2, 3)).to(weight_dtype)
                ...
                dynamic_weights = gen_dynamic_weights_sds(
                    ...
                    preds_gender_ij,
                    factor=args.factor1,
                    out_dtype=loss_img_z0_l2_ij.dtype,
                )

                loss_ij = (
                    loss_fair_ij
                    + args.weight_loss_img * dynamic_weights * loss_img_z0_l2_ij
                ...
                with torch.no_grad():
                    ...
                    loss_img_z0_l2_i[idxs_ij] = loss_img_z0_l2_ij.to(loss_img_z0_l2_i.dtype)
            ...
            loss_img_z0_l2_all = customized_all_gather(loss_img_z0_l2_i, accelerator)
            ...
            if accelerator.is_main_process:
                ...
                logs_i["loss_img_z0_l2"].append(loss_img_z0_l2_all)
                logs_i["loss"].append(loss_all)
            
            # process logs
            if accelerator.is_main_process:
                for key in ["loss_fair", "loss_face_realistic", "loss_img_z0_l2", "loss"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
                    else:
