...
def standardize_feature(x, eps=1e-6):
    """Per-sample standardization over (C,H,W) -> ~zero mean, ~unit variance/image.

    Optionally applied before the per-block h-space distance so blocks with very
    different activation scales contribute comparably to the averaged loss (each
    block's per-sample MSE then lands on an O(1) scale instead of being dominated
    by whichever block has the largest raw magnitude). Note this makes the term
    invariant to a per-image affine (scale+shift) of the features.
    """
...
def resolve_img_loss_feature_norm(h_block_norm="none", img_loss_feature_norm=None):
    """Resolve feature normalization mode for the image/h-space loss.

    Public CLI terms:
      - raw: use block outputs as-is before MSE
      - standard: standardize each block output per sample before MSE

    Internally we keep the older h_block_norm names for backward compatibility:
      - none: raw
      - std: standard
    """
    aliases = {
    ...
    def _canonical(value, arg_name):
        key = value.strip().lower() if isinstance(value, str) else value
        if key not in aliases:
            raise ValueError(
                f"Invalid {arg_name}={value!r}. Choose raw/none or standard/std."
            )
        return aliases[key]

    legacy_mode = _canonical(h_block_norm, "h_block_norm") or "none"
    public_mode = _canonical(img_loss_feature_norm, "img_loss_feature_norm")
    return public_mode or legacy_mode


def resolve_hspace_blocks(unet, spec):
    """Map a comma-separated block spec to an ordered list of (name, module).

    Recognised tokens (SD1.5 ``UNet2DConditionModel``):
      - "mid" / "mid_block"            -> unet.mid_block
      - "down.{i}" / "down_blocks.{i}" -> unet.down_blocks[i]
      - "up.{i}"   / "up_blocks.{i}"   -> unet.up_blocks[i]
      - "down" / "all_down"            -> every down block
      - "up"   / "all_up"              -> every up block
      - "all"                          -> down.0..N + mid + up.0..M  (default)

    This lets us add/remove which blocks feed the h-space (image-semantics
    preserving) loss purely from the CLI, e.g. --h_block_names "up,mid,down"
    or --h_block_names "mid,up.1,up.2".
    """
    n_down = len(unet.down_blocks)
    n_up = len(unet.up_blocks)

    tokens = [t.strip() for t in str(spec).split(",") if t.strip()]
    if len(tokens) == 0 or "all" in tokens:
        tokens = (
            [f"down.{i}" for i in range(n_down)]
            + ["mid"]
            + [f"up.{i}" for i in range(n_up)]
        )

    # expand group tokens (down / all_down / up / all_up) while preserving order
    expanded = []
    for tok in tokens:
        if tok in ("all_down", "down"):
            expanded += [f"down.{i}" for i in range(n_down)]
        elif tok in ("all_up", "up"):
            expanded += [f"up.{i}" for i in range(n_up)]
        else:
            expanded.append(tok)
    ...
    named = []
    for tok in ordered:
        if tok in ("mid", "mid_block"):
            named.append(("mid_block", unet.mid_block))
        elif tok.startswith("down"):
            i = int(tok.split(".")[1])
            if not (0 <= i < n_down):
                raise ValueError(f"down block index out of range: {tok!r} (have {n_down})")
            named.append((f"down_blocks.{i}", unet.down_blocks[i]))
        elif tok.startswith("up"):
            i = int(tok.split(".")[1])
            if not (0 <= i < n_up):
                raise ValueError(f"up block index out of range: {tok!r} (have {n_up})")
            named.append((f"up_blocks.{i}", unet.up_blocks[i]))
        else:
            raise ValueError(f"unknown h-space block token: {tok!r}")

    if len(named) == 0:
        raise ValueError(f"no valid h-space blocks resolved from spec {spec!r}")
    return named


class MultiBlockCapture:
    """Forward-hook wrapper that captures the outputs of several UNet blocks.

    Generalises ``MidBlockCapture`` to any set of down/mid/up blocks so the
    h-space L2 loss can match features at multiple depths, not only the
    mid_block bottleneck. ``self.last`` is a name->tensor dict, refreshed on
    every forward pass of the wrapped UNet.

    Note: down_blocks return ``(hidden_states, res_tuple)`` from ``forward`` —
    we keep ``hidden_states`` (element 0). mid/up blocks return a bare tensor.
    """

    def __init__(self, named_modules):
        # named_modules: list of (name, module) as produced by resolve_hspace_blocks
    ...
    def _make_hook(self, name):
        def _fn(module, inputs, output):
            self.last[name] = output[0] if isinstance(output, tuple) else output
        return _fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--h_block_names", type=str, default="all",
        help="Comma-separated UNet blocks whose outputs feed the h-space "
             "(image-semantics preserving) loss; their per-block distances are "
             "averaged. Tokens: 'mid', 'down.0'..'down.3', 'up.0'..'up.3', "
             "'all_down', 'all_up', 'all'. "
             "Default 'all' = down.0-3 + mid + up.0-3. "
             "Set 'mid' to reproduce the original mid_block-only behaviour.",
    )
    parser.add_argument(
        "--h_block_norm", type=str, default="none",
        choices=["none", "std", "raw", "standard"],
        help="Per-block feature normalization before the h-space distance. "
             "'none'/'raw' = raw features (default). "
             "'std'/'standard' = per-sample standardize (mean 0 / var 1 over C,H,W) each "
             "block output first, so blocks with different activation scales "
             "contribute comparably to the averaged loss.",
    )
    parser.add_argument(
        "--img_loss_feature_norm", type=str, default="raw",
        choices=["raw", "standard"],
        help="Feature normalization mode for the image semantics loss MSE. "
             "'raw' uses down/mid/up block outputs as-is; "
             "'standard' standardizes each block output per sample before MSE. "
             "Overrides --h_block_norm when set.",
    )
    ...
    args.h_block_norm = resolve_img_loss_feature_norm(
        args.h_block_norm,
        args.img_loss_feature_norm,
    )
    args.img_loss_feature_norm = "standard" if args.h_block_norm == "std" else "raw"
...
def main(args):
    ...
    folder_name = (
        ...
        f"_hBlk-{str(args.h_block_names).replace(',', '+')}"
        f"_hForm-{args.h_loss_form}"
        f"_hNorm-{args.img_loss_feature_norm}"
    ...
    def generate_image_w_gradient(
        ...
        # h-space capture across a configurable set of UNet blocks (block ablation).
        # Per-block distances are averaged into one term per denoising step.
        mid_capture_fine = None
        mid_capture_ref = None
        hspace_block_names = None
        if return_hspace_loss:
            spec = getattr(args, "h_block_names", "all")
            mid_capture_fine = MultiBlockCapture(resolve_hspace_blocks(which_unet, spec)).attach()
            hspace_block_names = mid_capture_fine.names
            if ref_is_same_unet:
                mid_capture_ref = mid_capture_fine
            else:
                mid_capture_ref = MultiBlockCapture(resolve_hspace_blocks(h_reference_unet, spec)).attach()
        ...
        try:
            ...
            for i, t in enumerate(timesteps[:steps_to_run]):
                ...
                latent_model_input = noise_scheduler.scale_model_input(latent_model_input, t)
                if return_hspace_loss:
                    mid_capture_fine.last = {}

                eps = which_unet(latent_model_input, t, encoder_hidden_states=prompt_embeds).sample
                # snapshot the fine features BEFORE the ref forward can overwrite them
                # (when ref_is_same_unet the two captures share one module set)
                h_fine = dict(mid_capture_fine.last) if return_hspace_loss else None
                eps = eps.to(weight_dtype_high_precision)
                ...
                if return_hspace_loss:
                    ...
                    if not h_fine or not h_ref:
                        raise RuntimeError("Failed to capture h-space during denoising.")

                    form = getattr(args, "h_loss_form", "raw")
                    block_norm = resolve_img_loss_feature_norm(
                        getattr(args, "h_block_norm", "none"),
                        getattr(args, "img_loss_feature_norm", None),
                    )
                    block_losses = []
                    for bname in hspace_block_names:
                        h_fine_cond = h_fine.get(bname)
                        h_ref_cond = h_ref.get(bname)
                        if h_fine_cond is None or h_ref_cond is None:
                            raise RuntimeError(
                                f"Failed to capture h-space for block '{bname}' during denoising."
                            )
                        # keep only the conditional half when uncond+cond are concatenated
                        ...
                        if h_fine_cond.shape != h_ref_cond.shape:
                            raise RuntimeError(
                                f"h-space shape mismatch for block '{bname}': "
                                f"fine={tuple(h_fine_cond.shape)} ref={tuple(h_ref_cond.shape)}"
                            )

                        hf = h_fine_cond.float()
                        hr = h_ref_cond.detach().float()
                        if block_norm == "std":
                            # standardize each block output so per-block scales match
                            hf = standardize_feature(hf)
                            hr = standardize_feature(hr)

                        if form == "cos":
                            block_loss = 1.0 - F.cosine_similarity(
                                hf.flatten(1),
                                hr.flatten(1),
                                dim=1,
                            )
                        else:
                            block_loss = ((hf - hr) ** 2).mean(dim=(1, 2, 3))
                        block_losses.append(block_loss)

                    # mean over the selected blocks -> a single [N] term for this step
                    loss_t = torch.stack(block_losses, dim=0).mean(dim=0)
