...
def main(args):
    ...
    eval_clip_image_processor = CLIPImageProcessor.from_pretrained(
        "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k",
    )
    eval_clip_tokenizer = CLIPTokenizer.from_pretrained(
        "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k",
    )
    eval_clip_model = CLIPModel.from_pretrained(
        "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k",
    )
    eval_clip_model.to(accelerator.device, dtype=weight_dtype)
    eval_clip_model.requires_grad_(False)
    eval_clip_model.eval()
    eval_clip_img_mean = torch.tensor(eval_clip_image_processor.image_mean).reshape([-1,1,1]).to(accelerator.device, dtype=weight_dtype)
    eval_clip_img_std = torch.tensor(eval_clip_image_processor.image_std).reshape([-1,1,1]).to(accelerator.device, dtype=weight_dtype)
    eval_clip_input_size = eval_clip_image_processor.crop_size["height"] if isinstance(eval_clip_image_processor.crop_size, dict) else eval_clip_image_processor.crop_size
    ...
    eval_dinov2.eval()
    # eval DINOv2 reuses the same ImageNet normalization as the training DINO (dinov2_img_mean/std)
    ...
    gender_classifier_test.to(accelerator.device, dtype=weight_dtype)
    ...
    def get_clip_feat_eval(images, normalize=True, to_high_precision=True):
        """image embeds from the evaluation CLIP model (CLIP-ViT-bigG-14), used for CLIP-T / CLIP-I.
        Differs from get_clip_feat, which uses the (smaller) training CLIP model.

        Args:
            images (torch.tensor): shape [N,3,H,W], in range [-1,1]
        """
        images_resized = transforms.Resize((eval_clip_input_size, eval_clip_input_size), antialias=True)(images)
        images_preprocessed = ((images_resized+1)*0.5 - eval_clip_img_mean) / eval_clip_img_std
        embeds = eval_clip_model.get_image_features(pixel_values=images_preprocessed.to(weight_dtype))
    ...
    def get_clip_text_feat_eval(prompts, normalize=True, to_high_precision=True):
        """text embeds from the evaluation CLIP model (CLIP-ViT-bigG-14), used for CLIP-T.
        ...
        if isinstance(prompts, str):
            prompts = [prompts]
        text_inputs = eval_clip_tokenizer(
            prompts,
            padding="max_length",
            max_length=eval_clip_tokenizer.model_max_length,
            truncation=True,
            return_tensors="pt",
        )
        text_inputs = {k: v.to(accelerator.device) for k, v in text_inputs.items()}
        embeds = eval_clip_model.get_text_features(**text_inputs)
    ...
    def get_dino_feat_eval(images, normalize=True, to_high_precision=True):
        """image embeds from the evaluation DINOv2 model (vit-g/14), used for DINO.
        Differs from get_dino_feat, which uses the (smaller) training DINOv2 model.

        Args:
            images (torch.tensor): shape [N,3,H,W], in range [-1,1]
        """
        images_resized = transforms.Resize((224, 224), antialias=True)(images)
        images_preprocessed = ((images_resized+1)*0.5 - dinov2_img_mean) / dinov2_img_std
    ...
    def residual_gender_logits(z0):
        ...
        residual_maps = {"woman": [], "man": []}   # per-timestep spatial error maps [n,H,W] (grad-carrying)
        attn_accum = torch.zeros(n, H, W, dtype=torch.float, device=z0.device)  # detached attention accumulator
        attn_count = 0

        for t in timesteps:
            t_batch = t.repeat(n)                                 # [n], same timestep for all samples
            eps = torch.randn_like(z0)                            # new eps per timestep
            zt = noise_scheduler.add_noise(z0, eps, t_batch)      # same eps & zt shared by woman/man
            zt_in = zt.to(weight_dtype)

            for cls, c, tok_idxs in prompts_cfg:
                attn_capture_ctx.store = []
                attn_capture_ctx.token_idxs = tok_idxs
                attn_capture_ctx.enabled = True
                eps_pred = scoring_unet(zt_in, t_batch, encoder_hidden_states=c).sample
                attn_capture_ctx.enabled = False
                captured = attn_capture_ctx.store
                attn_capture_ctx.store = []

                # channel-mean squared residual -> spatial error map [n,H,W] (keeps grad to z0)
                residual_maps[cls].append((eps_pred.float() - eps.float()).pow(2).mean(dim=1))
                ...
                for col, heads in captured:
                    ...
                    a = col.view(n, heads, s, s).float()                                    # [n, heads, s, s]
                    a = torch.nn.functional.interpolate(a, size=(H, W), mode="bilinear", align_corners=False)
                    attn_accum = attn_accum + a.mean(dim=1)                                 # mean over heads -> [n,H,W]
                    attn_count += 1
        ...
        E_woman = torch.stack(
            [(common_attn * rm).sum(dim=(1, 2)) for rm in residual_maps["woman"]], dim=1
        ).mean(dim=1)                                                                        # [n]
        E_man = torch.stack(
            [(common_attn * rm).sum(dim=(1, 2)) for rm in residual_maps["man"]], dim=1
        ).mean(dim=1)                                                                        # [n]
    ...
    def get_face_gender_test(face_chips, selector=None, fill_value=-1):
        ...
        if face_chips_w_faces.shape[0] == 0:
            ...
            logits_gender = gender_classifier_test(face_chips_w_faces)
            probs_gender = torch.softmax(logits_gender, dim=-1)
            preds_gender = probs_gender.max(dim=-1).indices
    ...
    def evaluate_process(which_text_encoder, which_unet, name, prompts, noises, current_global_step):
        ...
        for prompt_i, noises_i in itertools.zip_longest(prompts, noises):
            ...
            ################################################
            # eval fidelity / text-alignment metrics computed over ALL generated images:
            #   CLIP-T : cosine( eval-CLIP(generated image), eval-CLIP-text(prompt) )
            #   CLIP-I : cosine( eval-CLIP(generated image), eval-CLIP(original-SD image) )   [paired by noise]
            #   DINO   : cosine( eval-DINO(generated image), eval-DINO(original-SD image) )   [paired by noise]
            # images and images_ori are generated from the same noises_i in the same order, so
            # index k pairs generated[k] with images_ori[k].
            clip_feats_eval = get_clip_feat_eval(images)
            clip_feats_ori_eval = get_clip_feat_eval(images_ori)
            dino_feats_eval = get_dino_feat_eval(images)
            dino_feats_ori_eval = get_dino_feat_eval(images_ori)
            text_feats_eval = get_clip_text_feat_eval(prompt_i)

            sims_clip_t = (clip_feats_eval * text_feats_eval).sum(dim=-1)
            sims_clip_i = (clip_feats_eval * clip_feats_ori_eval).sum(dim=-1)
            sims_dino = (dino_feats_eval * dino_feats_ori_eval).sum(dim=-1)

            sims_clip_t_all = customized_all_gather(sims_clip_t, accelerator, return_tensor_other_processes=False)
            sims_clip_i_all = customized_all_gather(sims_clip_i, accelerator, return_tensor_other_processes=False)
            sims_dino_all = customized_all_gather(sims_dino, accelerator, return_tensor_other_processes=False)

            # keep only the first val_keep gathered images (see --val_images_per_prompt_total)
            sims_clip_t_all = sims_clip_t_all[:val_keep]
            sims_clip_i_all = sims_clip_i_all[:val_keep]
            sims_dino_all = sims_dino_all[:val_keep]
