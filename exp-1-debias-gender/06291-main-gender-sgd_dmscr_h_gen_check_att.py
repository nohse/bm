...
def parse_args(input_args=None):
    ...
    parser.add_argument("--sds_tau", type=float, default=0.00000003, help="temperature for -softmax on SDS losses")
...
def main(args):
    ...
    def sds_logits_from_images(
        ...
        res_f = eps_pred_f - eps                       # [BKE,C,H,W] (채널 차원 유지)
        res_m = eps_pred_m - eps
        res_r = (eps_pred_r - eps) if compute_realistic_sds else None
        if w_map is not None:
            ...
            # [fp16 underflow fix]
            #   원식 ((res·w)^2)는 w(~1/4096)를 먼저 곱한 뒤 제곱하므로 항이 ~1e-9 까지 작아져
            #   fp16 표현 하한(~6e-8) 밑으로 대량 underflow(실측 약 96% 가 0) → 에너지가 더 작아짐.
            #   해결: 제곱 '전에' hw 를 곱해 (w·hw)~O(1) 로 키워 안전하게 제곱한 뒤,
            #   공간 mean(÷hw) 후 알고 있는 상수 hw 로 한 번 더 나눠 동일한 값을 복원한다.
            #   항등식:  ((res·w·hw)^2).mean_c.mean_s / hw  ==  ((res·w)^2).mean_c.sum_s
            #   (값/의미 동일 = 실효 가중치는 여전히 w^2. fp16 underflow 만 제거.)
            hw = res_f.shape[-2] * res_f.shape[-1]
            wc = (w * hw).unsqueeze(1)                  # [BKE,1,H,W] 원소 평균 ~1 (O(1) → fp16 안전)
            sds_f_per = ((res_f * wc) ** 2).mean(dim=1).flatten(1).mean(dim=1).float() / hw  # [BKE]
            sds_m_per = ((res_m * wc) ** 2).mean(dim=1).flatten(1).mean(dim=1).float() / hw  # [BKE]
        else:
