...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="./outputs/gender_aaai/20260703-1001_gender_aaai_region-attn_skip-0pct_wImg-8.0_hBlk-up+mid_hForm-raw_hNorm-std_wRealFace-0.0_Th-0.2_lr-5e-05/ckpts/checkpoint_tmp-380",
    ...
    # [check_zscore] scale-only z-score of the gender signal g=sds_m-sds_f, plus per-step grad decomposition logging
    parser.add_argument("--use_zscore_logit",
                        type=lambda v: str(v).strip().lower() not in ("0", "false", "no", "off", "f", "n"),
                        default=True,
                        help="[check_zscore] gradient-pass SDS logit을 tau 대신 배치 std(g)로 나눔 (scale-only z-score). "
                             "std는 no-grad 타깃 패스에서 all-gather로 1회 계산해 detach 상수로 주입.")
    parser.add_argument("--grad_log_dir", type=str, default="./zscore_grad_logs",
                        help="[check_zscore] per-step per-loss gradient decomposition JSONL 저장 폴더 (main process only)")
    parser.add_argument("--grad_log_every", type=int, default=10,
                        help="[check_zscore] N 스텝마다 경사 분해 기록 (1=매 스텝, 기본 10). backward를 여러 번 더 하므로 느려짐.")
...
def main(args):
    ...
    # =========================================================================
    # [check_zscore] per-loss gradient decomposition logging (main process only)
    # Writes one JSON line per (step, chunk) with grad norms/shares/cosines of
    # fair vs img vs face, plus g / std / probs(z-score & tau) / targets / losses.
    # =========================================================================
    grad_log_path = None
    if accelerator.is_main_process and args.train_text_encoder:
        os.makedirs(args.grad_log_dir, exist_ok=True)
        _zc_tag = "zscore" if args.use_zscore_logit else "tau"
        grad_log_path = os.path.join(args.grad_log_dir, f"grad_log_main_{_zc_tag}.jsonl")
        with open(grad_log_path, "a") as _f:
            _f.write(json.dumps({
                "_meta": True, "use_zscore_logit": bool(args.use_zscore_logit),
                "sds_tau": float(args.sds_tau), "weight_loss_img": float(args.weight_loss_img),
                "weight_loss_face_realistic": float(args.weight_loss_face_realistic),
                "factor1": float(args.factor1), "region_mask_mode": str(args.region_mask_mode),
                "skip_final_steps_pct": float(args.skip_final_steps_pct),
                "sds_num_t": int(args.sds_num_t), "sds_num_eps": int(args.sds_num_eps),
                "train_GPU_batch_size": int(args.train_GPU_batch_size),
                "train_images_per_prompt_GPU": int(args.train_images_per_prompt_GPU),
                "lr": float(args.learning_rate),
            }) + "\n")

    def _zc_flat_grad(scalar, params):
    ...
    def _zc_log_grad_decomp(j, global_step, epoch, prompt, params,
                            loss_fair_ij, loss_hspace_ij, loss_face_ij, dynamic_weights,
                            idxs_valid, targets_ij, preds_ij, probs_ij, sds_f_ij, sds_m_ij,
                            zscore_denom, g_stats):
        if grad_log_path is None:
            return
        n_params = sum(p.numel() for p in params)
        Z = torch.zeros(n_params, device=params[0].device)
        # per-term grad vectors, exactly as each enters loss_ij.mean()
        gf = _zc_flat_grad(loss_fair_ij.mean(), params) if loss_fair_ij.requires_grad else Z
        img_term = (args.weight_loss_img * dynamic_weights * loss_hspace_ij).mean()
        gi = _zc_flat_grad(img_term, params) if img_term.requires_grad else Z
        face_term = (args.weight_loss_face_realistic * loss_face_ij).mean()
        gc = _zc_flat_grad(face_term, params) if getattr(face_term, "requires_grad", False) else Z
        nf = float(gf.norm().item()); ni = float(gi.norm().item()); nc = float(gc.norm().item())
        denom = nf + ni + nc + 1e-20
        # reference: fixed-tau probs on the SAME sds -> shows whether z-score de-saturates
        with torch.no_grad():
            lt = torch.stack([-sds_f_ij.float() / args.sds_tau, -sds_m_ij.float() / args.sds_tau], dim=1)
            lt = lt - lt.max(dim=1, keepdim=True).values
            probs_tau_male = torch.softmax(lt, dim=1)[:, 1]
        g_ij = (sds_m_ij.float() - sds_f_ij.float())
        rec = {
            "global_step": int(global_step), "epoch": int(epoch), "chunk": int(j),
            "prompt": str(prompt),
            "use_zscore": bool(args.use_zscore_logit),
            "divisor_used": float(zscore_denom if args.use_zscore_logit else args.sds_tau),
            "zscore_denom_std_skip0": float(zscore_denom), "sds_tau": float(args.sds_tau),
            "g_batch_mean_skip0": float(g_stats[0]), "g_batch_std_skip0": float(g_stats[1]),
            "g_batch_min_skip0": float(g_stats[2]), "g_batch_max_skip0": float(g_stats[3]),
            "n_valid": int(idxs_valid.numel()), "n_chunk": int(targets_ij.shape[0]),
            "targets": targets_ij.detach().cpu().tolist(),
            "preds_sds": preds_ij.detach().cpu().tolist(),
            "probs_male_zscore": probs_ij[:, 1].detach().cpu().float().tolist(),
            "probs_male_tau_ref": probs_tau_male.detach().cpu().tolist(),
            "g_chunk_skip50": g_ij.detach().cpu().tolist(),
            "dynamic_weights": [float(x) for x in dynamic_weights.detach().cpu().tolist()],
            "loss_fair_valid_mean": (float(loss_fair_ij[idxs_valid].mean().item()) if idxs_valid.numel() > 0 else None),
            "loss_hspace_mean": float(loss_hspace_ij.mean().item()),
            "loss_face_mean": float(loss_face_ij.mean().item()),
            "grad_norm_fair": nf, "grad_norm_img_weighted": ni, "grad_norm_face_weighted": nc,
            "grad_norm_total": float((gf + gi + gc).norm().item()),
            "share_fair": nf / denom, "share_img": ni / denom, "share_face": nc / denom,
            "cos_fair_img": _zc_cos(gf, gi), "cos_fair_face": _zc_cos(gf, gc),
        }
        with open(grad_log_path, "a") as _f:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                preds_gender_sds, probs_gender_sds, logits_gender_sds, sds_f_sds, sds_m_sds, _ = sds_logits_from_images(
                    images,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                ...
                # [check_zscore] scale-only z-score divisor = std(g = sds_m - sds_f) over the gathered no-grad batch.
                # Gathered + broadcast so every rank uses the SAME detached constant in the gradient pass below.
                # NOTE: this std is measured on fully-denoised (skip=0) images; the gradient pass uses skip-50 images,
                # so the g scale can differ — both are logged to JSON so the mismatch is observable.
                sds_f_all_sds = customized_all_gather(sds_f_sds, accelerator, return_tensor_other_processes=False)
                sds_m_all_sds = customized_all_gather(sds_m_sds, accelerator, return_tensor_other_processes=False)
                g_all_sds = (sds_m_all_sds.float() - sds_f_all_sds.float())
                zscore_denom_t = g_all_sds.std().clamp_min(1e-12).to(accelerator.device).float()
                torch.distributed.broadcast(zscore_denom_t, src=0)
                zscore_denom = float(zscore_denom_t.item())
                zscore_g_mean = float(g_all_sds.mean().item()); zscore_g_std = float(g_all_sds.std().item())
                zscore_g_min = float(g_all_sds.min().item()); zscore_g_max = float(g_all_sds.max().item())
                if accelerator.is_main_process:
            ...
            for j in range(N_backward):
                ...
                preds_gender_ij, probs_gender_ij, logits_gender_ij, sds_f_ij, sds_m_ij, sds_realistic_ij = sds_logits_from_images(
                    images_ij,
                    tau=(zscore_denom if args.use_zscore_logit else args.sds_tau),
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                    num_t=args.sds_num_t,
                    num_eps=args.sds_num_eps,
                ...
                # [check_zscore] per-loss gradient decomposition (main process only, before the real backward).
                # Uses torch.autograd.grad (retain_graph=True) so the subsequent accelerator.backward is unaffected.
                if (accelerator.is_main_process and args.train_text_encoder
                        and (global_step % max(1, args.grad_log_every) == 0)):
                    try:
                        _zc_log_grad_decomp(
                            j=j, global_step=global_step, epoch=epoch, prompt=prompt_i,
                            params=text_encoder_lora_params,
                            loss_fair_ij=loss_fair_ij, loss_hspace_ij=loss_hspace_ij,
                            loss_face_ij=loss_face_realistic_ij, dynamic_weights=dynamic_weights,
                            idxs_valid=idxs_valid, targets_ij=targets_ij, preds_ij=preds_gender_ij,
                            probs_ij=probs_gender_ij, sds_f_ij=sds_f_ij, sds_m_ij=sds_m_ij,
                            zscore_denom=zscore_denom,
                            g_stats=(zscore_g_mean, zscore_g_std, zscore_g_min, zscore_g_max),
                        )
                    except Exception as _zc_e:
                        accelerator.print(f"[check_zscore] grad decomp logging failed: {type(_zc_e).__name__}: {_zc_e}")
