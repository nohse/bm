...
def parse_args(input_args=None):
    ...
    parser.add_argument("--sds_tau", type=float, default=0.0000001, help="temperature for -softmax on SDS losses")
    # --- gap normalization (이 파일의 핵심): per-sample gap(sds_m - sds_f)을 배치 스케일로 정규화해
    #     softmax 입력을 O(1)로 만들고, 절대 SDS 크기와 무관하게 적당한 온도를 쓴다.
    #     이렇게 하면 (a) tau를 1e-7처럼 극단으로 안 내려도 prob가 잘 움직이고,
    #     (b) gradient 1/tau 증폭이 사라져 fp16 overflow(NaN)가 안 난다. ]
    parser.add_argument("--use_gap_norm", dest="use_gap_norm", action="store_true",
                        help="gap(sds_m - sds_f)을 배치 std로 정규화한 뒤 gap_tau로 softmax (이 파일 기본 ON)")
    parser.add_argument("--no_gap_norm", dest="use_gap_norm", action="store_false",
                        help="gap 정규화를 끄고 기존 -sds/sds_tau 방식을 사용")
    parser.set_defaults(use_gap_norm=True)
    parser.add_argument("--gap_tau", type=float, default=0.3,
                        help="gap 정규화 시 사용할 온도(정규화된 gap이 O(1)이라 0.1~0.5 권장)")
...
def main(args):
    ...
    # Raise the NCCL collective timeout from the 10-min default to 60 min. The eval-time
    # FR face-detection fallback (slow dlib CNN) makes per-rank eval time diverge; without
    # this, the faster rank's collective hits the 600s watchdog and SIGABRTs the whole run.
    ...
    folder_name = (
        ...
        + (f"_gapnorm-{args.gap_tau}" if getattr(args, "use_gap_norm", False) else f"_tau-{args.sds_tau}")
    )
    ...
    def sds_logits_from_images(
        ...
        # --- SDS = w로 마스킹한 residual의 에너지: (w · res)^2  (w는 sum-to-one) ---
        #   w = Norm(Attn): 음수 0클램프 후 공간 합이 1  (Σ_spatial w = 1)
        #   loss = || w ⊙ (eps_pred - eps) ||^2  → residual error에 w를 곱한 뒤 제곱.
        #   주의: err에 걸리는 실효 가중치는 w^2 이며 합은 1이 아님(attention이 뾰족할수록 ↑).
        # [gap파일 수정] SDS 계산을 fp32로: 작은 sds의 forward underflow(→0)와
        #   backward에서 1/tau로 증폭된 gradient의 fp16 overflow(→inf→NaN)를 모두 방지.
        #   큰 gradient는 fp32에 머물다가 w²로 줄어든 뒤 fp16 파라미터로 흘러감.
        res_f = (eps_pred_f - eps).float()             # [BKE,C,H,W], fp32
        res_m = (eps_pred_m - eps).float()
        res_r = (eps_pred_r - eps).float() if compute_realistic_sds else None
        if w_map is not None:
            # Norm(·): spatial 차원에서 합이 1이 되도록 정규화 (논문과 동일)
            w = w_map.to(res_f.dtype).clamp_min(0)
            w = w / w.flatten(1).sum(dim=1).clamp_min(1e-8).view(-1, 1, 1)  # Σ_spatial w == 1
            wc = w.unsqueeze(1)                         # [BKE,1,H,W] (채널 브로드캐스트)
            # residual error에 w를 곱한 뒤 제곱: (w · res)^2, 채널평균 후 공간합(에너지)
            sds_f_per = ((res_f * wc) ** 2).mean(dim=1).flatten(1).sum(dim=1)  # [BKE]
            sds_m_per = ((res_m * wc) ** 2).mean(dim=1).flatten(1).sum(dim=1)  # [BKE]
        else:
            # 마스크 없음: w 미적용 → 단순 공간 평균
            sds_f_per = (res_f ** 2).mean(dim=1).flatten(1).mean(dim=1)  # [BKE]
            sds_m_per = (res_m ** 2).mean(dim=1).flatten(1).mean(dim=1)  # [BKE]
        sds_r_per = None
        if compute_realistic_sds:
            # realistic SDS는 항상 full pixel 균일 평균 (w_map 미적용)
            sds_r_per = (res_r ** 2).mean(dim=1).flatten(1).mean(dim=1)  # [BKE]
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            # 2) 온도(tau) 결정: gap 정규화 ON이면 배치 스케일로 자동 정합
            if getattr(args, "use_gap_norm", False):
                # gap = sds_m - sds_f (성별 신호)를 배치 스케일 s로 나눠 O(1)로 만든 뒤
                # 적당한 온도 gap_tau로 softmax. 유효온도 tau_eff = s * gap_tau.
                #  → 절대 sds 크기(아주 작음)와 무관하게 prob가 잘 움직이고,
                #    1/tau 증폭이 사라져 fp16 overflow(NaN)도 안 난다.
                #  s는 "정규화 상수"이므로 detach (gradient는 흘리지 않음).
                G = (sds_m32 - sds_f32).detach()  # [B]
                # [gap파일 수정] s = "전형적 gap 크기" = mean(|gap|).
                #   std를 쓰면 모델이 한 성별로 붕괴해 gap이 다 비슷해질 때 std→0(→clamp 1e-8)
                #   → tau_eff 극소 → gradient 폭발(NaN). abs().mean()은 gap이 비슷해도
                #   그 공통 크기를 유지하므로 tau_eff가 합리적으로 유지됨.
                s = G.abs().mean().clamp_min(1e-8)
                gap_tau = float(getattr(args, "gap_tau", 0.3))
                tau32 = (s * gap_tau).to(torch.float32)    # 유효 온도(스칼라 텐서)
            else:
    ...
    def get_face(images, fill_value=-1):
        ...
        #
        # get_face_FR runs a slow dlib CNN (face_recognition, model="cnn") on every image
        # InsightFace misses, to improve detection recall for the eval gender-gap metric.
        # When the model degrades and emits non-face images, the NUMBER of misses differs
        # across ranks, so eval wall-time diverges and used to trip the 600s NCCL watchdog
        # -> SIGABRT. We KEEP this fallback (no change to detection/metrics) and instead
        # raise the NCCL collective timeout at Accelerator init so slow eval cannot abort.
        USE_FR_FALLBACK = True
        if USE_FR_FALLBACK and face_indicators_app.logical_not().sum() > 0:
