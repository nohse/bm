...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            # valid (non-abstained) count this process -> clean fair-loss mean-over-valid across microbatches
            num_valid_total = max(int((targets != -1).sum().item()), 1)
            for j in range(N_backward):
                ...
                # anchors unchanged (per-microbatch mean); fair = clean MEAN over valid images
                # (global per-process via num_valid_total) so abstained rows do not dilute it.
                loss_anchor_ij = (
                    args.weight_loss_img * dynamic_weights * loss_hspace_ij
                    + args.weight_loss_face_realistic * loss_face_realistic_ij
                )
                loss_backward = loss_anchor_ij.mean() + fair_row_ij.sum() / num_valid_total
