...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="weight for the attractive-only fairness loss = mean over valid images of the "
             "assigned-gender SDS denoising error. NOTE: the old CE form was amplified by ~1/sds_tau; "
             "this raw-MSE form is O(1), so retune against weight_loss_img / weight_loss_face_realistic.",
    )
    parser.add_argument(
        '--fair_correct_weight',
        default=1,
        type=float,
        help="multiplier on the fairness loss for images already at their assigned gender "
             "(Step-2 SDS pred == target). 0.0 = train only the flips (ratio-movers); "
             "~0.2 keeps a soft pull on correct images (more stable, small majority-gender headwind).",
    )
    ...
    parser.add_argument(
        "--h_loss_form", type=str, default="snorm",
        choices=["raw", "cos", "snorm"],
        help="h-space distance form during generation steps. "
             "'raw' = elementwise MSE. "
             "'cos' = 1 - cosine_similarity on flattened h (scale-invariant). "
             "'snorm' = standard-norm (standardize each mid-block output to zero-mean/"
             "unit-std per sample over (C,H,W)), then elementwise MSE (default).",
    )
...
def main(args):
    ...
    # (open_clip ViT-bigG-14, DINOv2 vit-g/14);
    ...
    # each loss weight (img / realistic-face / fair, plus the fair flip-gate weight).
    folder_name = (
        ...
        f"_wFair-{args.weight_loss_fair}"
        f"_fcw-{args.fair_correct_weight}"
        ...
        try:
            ...
            for i, t in enumerate(timesteps[:steps_to_run]):
                ...
                if return_hspace_loss:
                    ...
                    form = getattr(args, "h_loss_form", "snorm")
                    if form == "cos":
                        ...
                        # standard-norm: standardize each sample's mid-block output to
                        # zero-mean / unit-std over (C,H,W), then take elementwise MSE.
                        norm_dims = (1, 2, 3)
                        hf = h_fine_cond.float()
                        hr = h_ref_cond.detach().float()
                        hf = (hf - hf.mean(dim=norm_dims, keepdim=True)) / (
                            hf.std(dim=norm_dims, keepdim=True) + 1e-6
                        )
                        hr = (hr - hr.mean(dim=norm_dims, keepdim=True)) / (
                            hr.std(dim=norm_dims, keepdim=True) + 1e-6
                        )
                        loss_t = ((hf - hr) ** 2).mean(dim=norm_dims)
                    else:
    ...
    def get_face_gender(face_chips, selector=None, fill_value=-1):
        ...
        # Pin to float32 regardless of whether any face was detected. The no-face branch
        # above builds probs/logits with face_chips.dtype (fp16 under mixed precision) while
        # the has-face branch uses .float() (fp32). When ranks disagreed on face count, the
        # gathered probs_gender_mnet had MISMATCHED dtypes (fp16 vs fp32) and
        # torch.distributed.all_gather DEADLOCKED silently until the NCCL watchdog timeout
        # (~1h) -> SIGABRT. This is the actual cause of the step-200/400/600/... eval hangs.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                # valid FLIPs = images that must change gender (target != current SDS pred): the real debiasing budget
                _num_valid_flip = int(((targets_all != -1) & (targets_all != preds_gender_all_sds)).sum().item())
                accelerator.print(f"\tNum valid FLIPs (target!=pred, the actual ratio-movers): {_num_valid_flip}/{(targets_all!=-1).sum().item()}")
            ...
            for j in range(N_backward):
                ...
                preds_gender_ij, probs_gender_ij, logits_gender_ij, sds_f32_ij, sds_m32_ij, sds_realistic_ij = sds_logits_from_images(
                    images_ij,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                ...
                # === NEW fair loss: attractive-only, per-image ASSIGNED-gender SDS error ===
                # Replaces CrossEntropy on [-s_f/tau, -s_m/tau]. No softmax / tau / repulsion:
                # each valid image is pulled ONLY toward its assigned gender's manifold.
                idxs_valid = (targets_ij != -1).nonzero().view([-1])

                # absolute denoising error of the ASSIGNED class: male(1)->s_m, female(0)->s_f
                s_assigned_ij = torch.where(targets_ij == 1, sds_m32_ij, sds_f32_ij)  # fp32, differentiable

                # flip-gate: full weight on images that must CHANGE gender (the ratio-movers),
                # down-weight already-correct images via --fair_correct_weight. This removes the
                # majority-gender headwind and preserves within-gender diversity. "is flip" uses the
                # stable Step-2 full-denoise SDS pred, not the noisy partial-denoise one.
                preds_step2_ij = preds_gender_sds[idxs_ij]
                fair_gate_ij = torch.where(
                    preds_step2_ij != targets_ij,
                    torch.ones_like(s_assigned_ij),
                    torch.full_like(s_assigned_ij, float(args.fair_correct_weight)),
                )
                fair_row_ij = args.weight_loss_fair * fair_gate_ij * s_assigned_ij
                fair_row_ij = fair_row_ij.masked_fill(targets_ij == -1, 0.0)  # drop abstained rows

                # logging-only sentinel vector (>=0 for valid, -1 for abstained)
                loss_fair_ij = torch.ones(len(idxs_ij), dtype=weight_dtype, device=accelerator.device) * (-1)
                if idxs_valid.numel() > 0:
                    loss_fair_ij[idxs_valid] = (fair_gate_ij * s_assigned_ij)[idxs_valid].to(weight_dtype)
                ...
                # Both terms use a per-microbatch mean (÷ microbatch size), matching the
                # original debias.py convention so the grad-sync /N_backward composes into a
                # single per-process mean instead of dividing the fair term a second time.
                # (Abstained rows are 0 in fair_row_ij but still counted in the denominator,
                # same as debias.py.)
                loss_anchor_ij = (
                    args.weight_loss_img * dynamic_weights * loss_hspace_ij
                    + args.weight_loss_face_realistic * loss_face_realistic_ij
                )
                loss_backward = loss_anchor_ij.mean() + fair_row_ij.mean()
                accelerator.backward(loss_backward)

                # per-row total loss for logging only (loss_fair_ij keeps the -1 sentinel)
                loss_ij = loss_fair_ij + loss_anchor_ij
            ...
            if accelerator.is_main_process:
                # wandb (train): log ONLY the losses each step.
                for key in ["loss_fair", "loss_face_realistic", "loss_hspace", "loss"]:
                    if key not in logs_i:
                        continue
                    values = logs_i[key]
