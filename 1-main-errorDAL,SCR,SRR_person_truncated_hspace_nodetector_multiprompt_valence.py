...
# =====================================================================================
# MULTIPROMPT_VALENCE = the ATTMAP file with the CLASS ITSELF replaced. The class is no longer gender
...
# ------------------------------------------------------------------------------------------------
# FACE DETECTOR (--valence_face_gate, default none): DROPPED from the training path. A faceless but grim
#   alleyway HAS a valence. The errFD gate would restrict the 50/50 objective to the face-containing
#   subpopulation -- an invisible sample-selection bias, not a safety net. residual_face_indicators is
#   KEPT and reachable via --valence_face_gate errfd for the ablation. Removing the gate also
#   STRUCTURALLY kills the NCCL all_gather deadlock class that has bitten this project twice: the
#   gathered dtypes/shapes no longer depend on per-rank detection outcomes.
# CROSS-ATTENTION MAP: DROPPED from all three consumers. In order of force:
#   1. It is ALREADY off for the class error: --gender_attn_weight defaults to 'none' in the ATTMAP
#      file (despite that file's own header claiming 'attn'). Only the SRR region and SCR gate used it.
#   2. It cannot be EXPRESSED here: _find_word_token_indices needs a CONTIGUOUS token run, but axis 2's
#      content words are "clothing ... actions" and axis 4's are "scene ... tone" (non-adjacent), and
#      asking for "clothing and actions" would average the function token "and" into the region map.
#   3. It would not be COMMENSURABLE: the 8 valence prompts have different token lengths (vs woman/man's
#      identical 5), and the maps are accumulated RAW and normalised to sum-to-1 only once at the end,
#      so the common map would be dominated by whichever prompt yields larger raw attention columns.
#   4. It would not MEAN anything: the 4 axes localise to DIFFERENT places (face / body / background /
#      whole frame), so their mean smears toward uniform. The woman/man map already had only ~2.8x
#      max/min contrast; averaging 4 axes whose deviations sit elsewhere collapses that to ~1.3x. Axis 4
#      has no localisable content noun at all, by construction.
#   The scoring UNet therefore keeps its DEFAULT (SDPA) attention processor. This is also a real speedup:
#   CrossAttnCaptureProcessor unconditionally took the slow explicit get_attention_scores + bmm path,
#   even when capture was disabled -- so NOT installing it is strictly faster than disabling it.
...
# Run-folder tag: _val-<P>ax-<pooling>[-uncal]  (uncal = the calibration constants were left at defaults).
# =====================================================================================
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=[
            "expr::A photo of positive facial expressions::A photo of negative facial expressions",
            "cloth::A photo of positive clothing and actions::A photo of negative clothing and actions",
            "bg::A photo of a background with a positive atmosphere::A photo of a background with a negative atmosphere",
        ...
        help="the P contrastive prompt PAIRS that jointly define the binary class, each as "
             "'name::POSITIVE prompt::NEGATIVE prompt'. Use MINIMAL CONTRASTS (only the polarity word "
             "differs): the per-axis gap g_p = E_neg - E_pos is a WITHIN-pair difference, so each axis's "
             "baseline error level (prompt length, rarity) cancels EXACTLY -- but only if the pair is "
             "otherwise identical. Class order is [negative=0, positive=1].",
    )
    parser.add_argument(
        '--valence_pooling',
        type=str,
    ...
    parser.add_argument(
        ...
        default="none",
        choices=["none", "errfd"],
        help="whether the residual-error FACE detector still gates the training loss. 'none' (default): it "
             "does NOT -- a faceless but grim alleyway HAS a valence, and gating would silently restrict "
             "the 50/50 objective to the face-containing subpopulation (an invisible sample-selection "
             "bias). It also removes the NCCL all_gather deadlock class, since the gathered dtypes/shapes "
             "no longer depend on per-rank detection outcomes. 'errfd': keep the gender file's behaviour "
             "(residual_face_indicators), for the ablation.",
    )
    parser.add_argument(
...
def main(args):
    ...
    # Each --valence_axes entry is 'name::POSITIVE prompt::NEGATIVE prompt'. Class order [neg=0, pos=1].
    valence_axes = []
    for spec in args.valence_axes:
        parts = [p.strip() for p in spec.split("::")]
        if len(parts) != 3 or not all(parts):
            raise ValueError(
                f"--valence_axes entry must be 'name::POSITIVE prompt::NEGATIVE prompt', got {spec!r}"
            )
        valence_axes.append({"name": parts[0], "pos": parts[1], "neg": parts[2]})
    ...
    _val_tag = f"_val-{P_axes}ax-{args.valence_pooling}{'-uncal' if _valence_uncal else ''}"
    folder_name = (
    ...
    # VALENCE: the capturing processor is NOT installed AT ALL.
    # Two separate reasons, and the second one is easy to miss:
    #  1. The map is meaningless for these axes -- they localise to different places (face / body /
    #     background / whole frame), so their common map smears toward uniform; axis 4 ("overall tone") has
    #     no localisable content noun at all; the content words of axes 2 and 4 are NON-CONTIGUOUS so
    #     _find_word_token_indices cannot even express them; and the 8 prompts have different token lengths
    #     while the maps are accumulated RAW and normalised only once at the end, so the longest prompt
    #     would dominate. See the header.
    #  2. NOT INSTALLING IT IS STRICTLY FASTER THAN INSTALLING-AND-DISABLING IT. CrossAttnCaptureProcessor
    #     unconditionally takes the explicit get_attention_scores + bmm path; `attn_capture_ctx.enabled`
    #     only guards the capture SIDE-EFFECT, not the attention math. Leaving the processor uninstalled
    #     keeps diffusers' default (SDPA/flash) attention on the scoring UNet, which is what the 2P-prompt
    #     scorer needs to stay affordable.
    # (CrossAttnCaptureProcessor and attn_capture_ctx above are left defined but UNUSED, so the gender
    # scorer they belong to stays readable/diffable against the attmap file.)
    if args.factor2 != 1.0:
        logger.warning(
    ...
    def _valence_axis_errors(z0):
        ...
        Returns E [n,P,2] (fp32), class order [neg=0, pos=1]:
            E[i,p,c] = mean_t mean_pixels || eps_pred(z_t, t, c_{p,c}) - eps ||^2
        over --residual_num_timesteps timesteps linspaced in [--residual_t_min, --residual_t_max].

        SPATIAL REDUCTION is a UNIFORM MEAN -- no attention weighting. For a scene-level class there is no
        coherent region to weight by (see the header). Note the gender file's --gender_attn_weight already
        DEFAULTED to 'none', so a uniform mean is also what it was actually doing.
        ...
        runs ONCE per prompt (2P forwards) rather than 2*P*K sequential ones.
        """
        n = z0.shape[0]
        ...
        for ax in valence_axes:
            ...
            per_class = []
            for embeds in (ax["neg_embeds"], ax["pos_embeds"]):          # class order [neg=0, pos=1]
                c = embeds.expand(n * K, -1, -1)
                eps_pred = scoring_unet(zt_all, t_all, encoder_hidden_states=c).sample
                # uniform mean over channels+pixels, then over the K timesteps -> [n]. The .float() is what
                # pins the whole valence pipeline to fp32; do not remove it.
                e = (eps_pred.float() - eps_all.float()).pow(2).mean(dim=(1, 2, 3)).view(n, K).mean(dim=1)
                per_class.append(e)
            cols.append(torch.stack(per_class, dim=1))                   # [n,2]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):
            ...
            with torch.no_grad():
                ...
                # VALENCE: every image has a valence, so by default EVERY image is scored and participates
                # (--valence_face_gate none). The errFD face gate is only built when explicitly asked for.
                ...
                if accelerator.is_main_process:
                    # No -1 filter: every image is scored, so the denominator is every image.
                    p_pos = probs_val_all[:, 1]
                    valence_gap = ((p_pos >= 0.5).float().mean() - (p_pos < 0.5).float().mean()).item()
                    logs_i["valence_gap"].append(valence_gap)
                    logs_i["valence_gap_abs"].append(abs(valence_gap))
                    logs_i["valence_pred_between_0.2_0.8"].append(
                        ((p_pos >= 0.2) * (p_pos <= 0.8)).float().mean().item()
                    )
                    ...
                    for _p, _ax in enumerate(valence_axes):
                        _g = gaps_val_all[:, _p]
                        logs_i[f"gap_{_ax['name']}_mean"].append(_g.mean().item())
                        logs_i[f"gap_{_ax['name']}_std"].append(_g.std().item() if _g.numel() > 1 else 0.0)
                        if _g.numel() > 1 and _g.std() > 0 and score_val_all.std() > 0:
                            _c = torch.corrcoef(torch.stack([_g.float(), score_val_all.float()]))[0, 1].item()
                        else:
                            _c = 0.0
