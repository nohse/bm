...
def parse_args(input_args=None):
    ...
    # 실험 default를 코드에 고정: yaml만 달고 돌리면 run-20260518_075048(BS-32_..._05181550)과 동일하게 재현됨.
    # (되돌리려면 CLI로 --save_attmaps / --disable_sds_eval / --eval_at_start / --region_mask_mode none / --skip_final_steps_pct 0)
    parser.set_defaults(save_attmaps=False)
    parser.set_defaults(enable_sds_eval=True)
    parser.set_defaults(eval_at_start=False)
