...
# =========================
# Implicit (pos/neg) prompt groups
# =========================
IMPLICIT_ASPECT_KEYS = ["facial", "dress_action", "background", "overall"]
IMPLICIT_PROMPT_GROUPS = {
#   "facial": {
#         "positive": "a photo of a person with a friendly, relaxed facial expression, such as a gentle smile",
#         "negative": "a photo of a person with a tense, upset facial expression, such as a frown or glare",
#     },
#     "dress_action": {
#         "positive": "a photo of people in neat clothing doing calm, cooperative, or helpful actions",
#         "negative": "a photo of people in messy clothing doing aggressive, careless, or disruptive actions",
#     },
#     "background": {
#         "positive": "a photo of a clean, bright, orderly background that feels safe and pleasant",
#         "negative": "a photo of a dark, cluttered, disorderly background that feels unsafe or unpleasant",
#     },
#     "overall": {
#         "positive": "a photo of an overall calm, safe, and welcoming scene",
#         "negative": "a photo of an overall tense, unsafe, and unsettling scene",
#     },
    # "facial": {
    #     "positive": "people with positive facial expressions such as smiles and relaxed eyes",
    #     "negative": "people with negative facial expressions such as frowns, anger, or discomfort",
    # },
    # "dress_action": {
    #     "positive": "people wearing clean, neat, professional attire and performing helpful actions",
    #     "negative": "people wearing messy, inappropriate clothing and showing aggressive or careless actions",
    # },
    # "background": {
    #     "positive": "a well-lit, tidy, and harmonious background",
    #     "negative": "a dim, cluttered, chaotic, or damaged background",
    # },
    # "overall": {
    #     "positive": "an overall scene with a calm, safe, and welcoming atmosphere",
    #     "negative": "the overall scene feels tense, unsafe, and hostile atmosphere",
    # },
    "facial": {
        "positive": "a photo of positive facial expressions",
        "negative": "a photo of negative facial expressions",
    },
    "dress_action": {
        "positive": "a photo of positive clothing and actions",
        "negative": "a photo of negative clothing and actions",
    },
    "background": {
        "positive": "a photo of a background with a positive atmosphere",
        "negative": "a photo of a background with a negative atmosphere",
    },
    "overall": {
        "positive": "a photo of a scene with an overall positive mood",
        "negative": "a photo of a scene with an overall negative mood",
    },
}
IMPLICIT_ITEMS = [
    {"aspect": aspect, "polarity": polarity, "text": IMPLICIT_PROMPT_GROUPS[aspect][polarity]}
    for aspect in IMPLICIT_ASPECT_KEYS
    for polarity in ("positive", "negative")
]
IMPLICIT_PROMPTS = [item["text"] for item in IMPLICIT_ITEMS]
IMPLICIT_POS_IDXS = [i for i, item in enumerate(IMPLICIT_ITEMS) if item["polarity"] == "positive"]
IMPLICIT_NEG_IDXS = [i for i, item in enumerate(IMPLICIT_ITEMS) if item["polarity"] == "negative"]
IMPLICIT_ASPECT_TO_IDXS = {
    aspect: {
        "positive": next(i for i, item in enumerate(IMPLICIT_ITEMS) if item["aspect"] == aspect and item["polarity"] == "positive"),
        "negative": next(i for i, item in enumerate(IMPLICIT_ITEMS) if item["aspect"] == aspect and item["polarity"] == "negative"),
    }
    for aspect in IMPLICIT_ASPECT_KEYS
}

# Hard-coded attmap token keywords used in SDS attention capture.
IMPLICIT_ATTMAP_KEYWORDS = {
    "facial": {
        "positive": ["facial", "expressions"],
        "negative": ["facial", "expressions"],
    },
    "dress_action": {
        "positive": ["clothing", "actions"],
        "negative": ["clothing", "actions"],
    },
    "background": {
        "positive": ["background"],
        "negative": ["background"],
    ...
    "overall": {
        "positive": ["overall", "positive", "mood"],
        "negative": ["overall", "negative", "mood"],
    },
}
...
def plot_in_grid(
    ...
    save_to,
    preds_implicit=None,
    pred_class_probs_implicit=None,
):
    """
    images: torch tensor in shape of [N,3,H,W], in range [-1,1]
    """
    # reorder by implicit class probability (positive first, then negative)
    if preds_implicit is not None and pred_class_probs_implicit is not None:
        idxs_pos = (preds_implicit == 0).nonzero(as_tuple=False).view([-1])
        probs_pos = pred_class_probs_implicit[idxs_pos]
        idxs_pos = idxs_pos[probs_pos.argsort(descending=True)]

        idxs_neg = (preds_implicit == 1).nonzero(as_tuple=False).view([-1])
        probs_neg = pred_class_probs_implicit[idxs_neg]
        idxs_neg = idxs_neg[probs_neg.argsort(descending=True)]

        idxs_unknown = (preds_implicit == -1).nonzero(as_tuple=False).view([-1])

        images_to_plot = []
        idxs_reordered = torch.cat([idxs_pos, idxs_neg, idxs_unknown])
    else:
    ...
    for idx in idxs_reordered:
        img = images[idx]
        pred_implicit = preds_implicit[idx] if preds_implicit is not None else torch.tensor(-1, device=images.device)
        pred_class_prob_implicit = (
            pred_class_probs_implicit[idx] if pred_class_probs_implicit is not None else torch.tensor(0.0, device=images.device)
        )

        if pred_implicit == 0:
            border_color = "blue"
        elif pred_implicit == 1:
            border_color = "red"
        else:
            border_color = "white"
        ...
        if pred_class_probs_implicit is not None and pred_class_prob_implicit.item() < 1:
            img_pil_draw.rectangle([(0,0),(50,(1-pred_class_prob_implicit.item())*512)], fill="white", outline=None)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="../data/1-prompts/occupation_wino_ori.json",
    ...
    parser.add_argument(
        ...
        help="zero-shot implicit classifier로 사용할 CLIP 모델 이름"
    )
    parser.add_argument(
    ...
    parser.add_argument("--region_mask_mode", type=str, default="attn",
                        choices=["none", "attn"],
                        help="SDS 및 backprop에서 사용할 영역 마스킹 방식 선택. 'attn'은 기존 어텐션 맵, 'none'은 전체. (실험 default: attn)")
    parser.add_argument(
        "--save_attmaps",
        dest="save_attmaps",
        action="store_true",
        help="평가 시 aspect별 attmap/overlay 이미지를 저장합니다.",
    )
    parser.add_argument(
        "--no_save_attmaps",
        dest="save_attmaps",
    ...
    # 실험 default를 코드에 고정: yaml만 달고 돌리면 현재 실험과 동일하게 재현됨.
    # (되돌리려면 CLI로 --save_attmaps / --disable_sds_eval / --region_mask_mode none 사용)
    parser.set_defaults(save_attmaps=False)
    parser.set_defaults(enable_sds_eval=True)
...
def main(args):
    ...
    # ---- zero-shot implicit text embedding 미리 계산 (pos/neg, 4 aspects) ----
    implicit_prompts = IMPLICIT_PROMPTS
    with torch.no_grad():
        text_inputs = clip_processor(
            text=implicit_prompts,
            return_tensors="pt",
            padding=True
        )
        text_inputs = {k: v.to(accelerator.device) for k, v in text_inputs.items()}
        implicit_text_features = clip_model.get_text_features(**text_inputs)
        implicit_text_features = F.normalize(implicit_text_features, dim=-1)
    ...
    # =========================
    # [MODIFIED] SDS-based classifier utilities (implicit pos/neg)
    ...
    def sds_logits_from_images(
        ...
        """
        - region_mask_mode == 'attn': SDS 계산 시 aspect별 공통 attmap을 내부에서 추출하여 가중
        ...
        # --- 텍스트 임베딩 (8 prompts) ---
        prompt_embeds = _text_embeds(IMPLICIT_PROMPTS, sds_text_encoder).to(weight_dtype)  # [C,seq,hidden]
        pe_realistic = None
        if compute_realistic_sds:
            pe_realistic = _text_embeds([realistic_prompt] * BKE, sds_text_encoder).to(weight_dtype)
        num_prompts = prompt_embeds.shape[0]
        attmaps_bke_per_prompt = [None] * num_prompts
        errs_per_prompt = []
        ...
        def _run_unet_with_optional_attn(
            ...
            if args.region_mask_mode == "attn" and args.use_attn_weight:
                keywords_i = IMPLICIT_ATTMAP_KEYWORDS[aspect_i][polarity_i]
                tok_pos_i = find_token_positions(tokenizer, prompt_text_i, keywords=keywords_i)
                if len(tok_pos_i) > 0:
            ...
            try:
                eps_pred_i = sds_unet(zt.to(weight_dtype), t_vec, encoder_hidden_states=prompt_embeds_i).sample.to(weight_dtype)
                if capture is not None:
                    att_map_i = capture.aggregated_map()
            finally:
                if capture is not None:
                    capture.clear()
            return eps_pred_i, _resize_attn_map(att_map_i)

        # --- UNet 예측 및 prompt별 에러/attmap 수집 ---
        for c in range(num_prompts):
            pe_c = prompt_embeds[c].unsqueeze(0).expand(BKE, -1, -1).contiguous()
            item_c = IMPLICIT_ITEMS[c]
            eps_pred_c, att_map_c = _run_unet_with_optional_attn(
                pe_c,
                IMPLICIT_PROMPTS[c],
                item_c["aspect"],
                item_c["polarity"],
            ...
            attmaps_bke_per_prompt[c] = att_map_c

        # --- aspect별 공통 attmap(positive/negative 평균) 생성 ---
        aspect_wmaps_bke = {}
        aspect_attmaps_mean = {}
        if args.region_mask_mode == "attn" and args.use_attn_weight:
            for aspect in IMPLICIT_ASPECT_KEYS:
                pos_idx = IMPLICIT_ASPECT_TO_IDXS[aspect]["positive"]
                neg_idx = IMPLICIT_ASPECT_TO_IDXS[aspect]["negative"]
                a_pos = attmaps_bke_per_prompt[pos_idx]
                a_neg = attmaps_bke_per_prompt[neg_idx]
                if a_pos is None and a_neg is None:
                    continue
                if a_pos is None:
                    a_common = a_neg
                elif a_neg is None:
                    ...
                    a_common = 0.5 * (a_pos + a_neg)
                if a_common is None:
                    continue
                a_common = _resize_attn_map(a_common)
                if a_common is None:
                    continue
                aspect_wmaps_bke[aspect] = a_common
                aspect_attmaps_mean[aspect] = (
                    a_common.float()
        ...
        # --- aspect별 공통 attmap을 각 prompt의 SDS에 가중 곱 ---
        sds_per_prompt = []
        for c in range(num_prompts):
            item_c = IMPLICIT_ITEMS[c]
            err_c = errs_per_prompt[c]
            w_map = aspect_wmaps_bke.get(item_c["aspect"], None)
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            # 2) positive/negative 합산 (SDS 값 기준)
            sds_pos32 = sds_per_prompt32[:, IMPLICIT_POS_IDXS].sum(dim=1)  # [B]
            sds_neg32 = sds_per_prompt32[:, IMPLICIT_NEG_IDXS].sum(dim=1)  # [B]
            sds_real32 = None
            ...
            logits32 = torch.stack([-sds_pos32 / tau32, -sds_neg32 / tau32], dim=1)  # [B,2], fp32
        ...
        if bad_rows.any():
            ...
            print("sds_pos32[min,max]:", float(sds_pos32.min()), float(sds_pos32.max()))
            print("sds_neg32[min,max]:", float(sds_neg32.min()), float(sds_neg32.max()))
        ...
        if return_attmaps:
            return preds, probs32, logits32, sds_pos32, sds_neg32, sds_per_prompt32, sds_real32, aspect_attmaps_mean
        return preds, probs32, logits32, sds_pos32, sds_neg32, sds_per_prompt32, sds_real32

    def clip_implicit_classifier(
        images: torch.Tensor,
        ...
        반환:
            preds:  [B] int64       (0: positive, 1: negative)
            probs:  [B, 2] float32
            logits: [B, 2] float32
            prompt_logits: [B, 8] float32 (각 prompt logits)
        """
        ...
        with torch.no_grad():
            ...
            prompt_logits = (image_features @ text_features.t()) * logit_scale  # [B,8]

            # 6) positive/negative 합산 -> 최종 logits
            pos_sum = prompt_logits[:, IMPLICIT_POS_IDXS].sum(dim=1)
            neg_sum = prompt_logits[:, IMPLICIT_NEG_IDXS].sum(dim=1)
            logits = torch.stack([pos_sum, neg_sum], dim=1)
    ...
    def generate_dynamic_targets(probs, target_ratio=0.5, w_uncertainty=False):
        ...
        Args:
            ...
            target_ratio (float): target distribution, the percentage of class 1 (negative)
        ...
        for prompt_i, noises_i in itertools.zip_longest(prompts, noises):
            if accelerator.is_main_process:
                logs_i = {
                    "implicit_gap": [],
                    "implicit_gap_abs": [],
                    "implicit_pred_between_0.2_0.8": [],
                }
                if enable_sds_eval:
                    logs_i["implicit_gap_abs_sds"] = []
                log_imgs_i = {}
            ...
            aspect_attmaps_ori_all = {}
            if enable_sds_eval:
                preds_implicit_ori_sds, probs_implicit_ori_sds, _, _, _, _, _, aspect_attmaps_ori = sds_logits_from_images(
                    images_ori,
                    tau=args.sds_tau,
            ...
            preds_implicit_ori, probs_implicit_ori, _, _ = clip_implicit_classifier(
                images_ori,
                clip_model=clip_model,
                clip_processor=clip_processor,
                text_features=implicit_text_features,
                device=accelerator.device,
            )
            images_ori_all = customized_all_gather(images_ori, accelerator, return_tensor_other_processes=False)
            preds_implicit_ori_all = customized_all_gather(preds_implicit_ori, accelerator, return_tensor_other_processes=False)
            probs_implicit_ori_all = customized_all_gather(probs_implicit_ori, accelerator, return_tensor_other_processes=False)
            if enable_sds_eval:
                preds_implicit_ori_all_sds = customized_all_gather(preds_implicit_ori_sds, accelerator, return_tensor_other_processes=False)
                probs_implicit_ori_all_sds = customized_all_gather(probs_implicit_ori_sds, accelerator, return_tensor_other_processes=False)
                if args.region_mask_mode == "attn" and isinstance(aspect_attmaps_ori, dict):
                    for aspect in IMPLICIT_ASPECT_KEYS:
                        att_i = aspect_attmaps_ori.get(aspect, None)
                        if att_i is not None:
                            aspect_attmaps_ori_all[aspect] = customized_all_gather(att_i, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                plot_in_grid(
                    images_ori_all,
                    save_to_ori,
                    preds_implicit=preds_implicit_ori_all,
                    pred_class_probs_implicit=probs_implicit_ori_all.max(dim=-1).values,
                )
                if enable_sds_eval:
                    ...
                    plot_in_grid(
                        images_ori_all,
                        save_to_ori_sds,
                        preds_implicit=preds_implicit_ori_all_sds,
                        pred_class_probs_implicit=probs_implicit_ori_all_sds.max(dim=-1).values,
                    )
                log_imgs_i["img_ori"] = [save_to_ori]
                if enable_sds_eval and args.save_attmaps and len(aspect_attmaps_ori_all) > 0:
                    att_save_dir = os.path.join(args.imgs_save_dir, "eval_attmaps")
                    for aspect, att in aspect_attmaps_ori_all.items():
                        att_prefix = f"eval_{name}_{global_step}_{sanitize_filename(prompt_i)}_ori_att_{aspect}"
                        save_attmaps(att, att_save_dir, att_prefix)
                        save_attmaps_with_overlay(att, images_ori_all, att_save_dir, att_prefix, alpha=0.45)
                        att_preview = os.path.join(att_save_dir, f"{att_prefix}_0_overlay.png")
                        if os.path.exists(att_preview):
                            log_imgs_i[f"attmap_ori_overlay_{aspect}"] = [att_preview]
            ...
            aspect_attmaps_gen_all = {}
            if enable_sds_eval:
                preds_implicit_sds, probs_implicit_sds, _, _, _, _, _, aspect_attmaps_gen = sds_logits_from_images(
                    images,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
            ...
            preds_implicit, probs_implicit, _, _ = clip_implicit_classifier(
                images,
                clip_model=clip_model,
                clip_processor=clip_processor,
                text_features=implicit_text_features,
            ...
            preds_implicit_all = customized_all_gather(preds_implicit, accelerator, return_tensor_other_processes=False)
            probs_implicit_all = customized_all_gather(probs_implicit, accelerator, return_tensor_other_processes=False)
            if enable_sds_eval:
                preds_implicit_all_sds = customized_all_gather(preds_implicit_sds, accelerator, return_tensor_other_processes=False)
                probs_implicit_all_sds = customized_all_gather(probs_implicit_sds, accelerator, return_tensor_other_processes=False)
                if args.region_mask_mode == "attn" and isinstance(aspect_attmaps_gen, dict):
                    for aspect in IMPLICIT_ASPECT_KEYS:
                        att_i = aspect_attmaps_gen.get(aspect, None)
                        if att_i is not None:
                            aspect_attmaps_gen_all[aspect] = customized_all_gather(att_i, accelerator, return_tensor_other_processes=False)
            ...
            if accelerator.is_main_process:
                ...
                plot_in_grid(
                    images_all,
                    save_to_generated,
                    preds_implicit=preds_implicit_all,
                    pred_class_probs_implicit=probs_implicit_all.max(dim=-1).values,
                )
                if enable_sds_eval:
                    ...
                    plot_in_grid(
                        ...
                        preds_implicit=preds_implicit_all_sds,
                        pred_class_probs_implicit=probs_implicit_all_sds.max(dim=-1).values,
                    )
                log_imgs_i["img_generated"] = [save_to_generated]
                if enable_sds_eval and args.save_attmaps and len(aspect_attmaps_gen_all) > 0:
                    att_save_dir = os.path.join(args.imgs_save_dir, "eval_attmaps")
                    for aspect, att in aspect_attmaps_gen_all.items():
                        att_prefix = f"eval_{name}_{global_step}_{sanitize_filename(prompt_i)}_generated_att_{aspect}"
                        save_attmaps(att, att_save_dir, att_prefix)
                        save_attmaps_with_overlay(att, images_all, att_save_dir, att_prefix, alpha=0.45)
                        att_preview = os.path.join(att_save_dir, f"{att_prefix}_0_overlay.png")
                        if os.path.exists(att_preview):
                            log_imgs_i[f"attmap_generated_overlay_{aspect}"] = [att_preview]

            if accelerator.is_main_process:
                probs_tmp = probs_implicit_all[(probs_implicit_all != -1).all(dim=-1)]
                implicit_gap = (((probs_tmp[:, 1] >= 0.5) * (probs_tmp[:, 1] <= 1)).float().mean() - ((probs_tmp[:, 1] >= 0) * (probs_tmp[:, 1] <= 0.5)).float().mean()).item()
                implicit_pred_between_02_08 = ((probs_tmp[:, 1] >= 0.2) * (probs_tmp[:, 1] <= 0.8)).float().mean().item()
                logs_i["implicit_gap"].append(implicit_gap)
                logs_i["implicit_gap_abs"].append(abs(implicit_gap))
                logs_i["implicit_pred_between_0.2_0.8"].append(abs(implicit_pred_between_02_08))
                if enable_sds_eval:
                    probs_tmp_sds = probs_implicit_all_sds[(probs_implicit_all_sds != -1).all(dim=-1)]
                    implicit_gap_sds = (((probs_tmp_sds[:, 1] >= 0.5) * (probs_tmp_sds[:, 1] <= 1)).float().mean() - ((probs_tmp_sds[:, 1] >= 0) * (probs_tmp_sds[:, 1] <= 0.5)).float().mean()).item()
                    logs_i["implicit_gap_abs_sds"].append(abs(implicit_gap_sds))
        ...
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):            
            ...
            with torch.no_grad():
                ...
                for j in range(N):
                    ...
                    images.append(images_ij)
                images = torch.cat(images)
                preds_implicit_sds, probs_implicit_sds, logits_implicit_sds, _, _, _, _ = sds_logits_from_images(
                    images,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                ...
                preds_implicit, probs_implicit, logits_implicit, _ = clip_implicit_classifier(
                    images,
                    clip_model=clip_model,
                    clip_processor=clip_processor,
                    text_features=implicit_text_features,
                ...
                if accelerator.is_main_process:
                    if step % args.train_plot_every_n_iter == 0:
                        ...
                        plot_in_grid(images_all, save_to, preds_implicit=preds_implicit_all, pred_class_probs_implicit=probs_implicit_all.max(dim=-1).values)
                        save_to = os.path.join(args.imgs_save_dir, f"train-{global_step}_generated_sds.jpg")
                        plot_in_grid(images_all, save_to, preds_implicit=preds_implicit_all_sds, pred_class_probs_implicit=probs_implicit_all_sds.max(dim=-1).values)
                ...
                if accelerator.is_main_process:
                    ...
                    logs_i["implicit_gap"].append(implicit_gap)
                    logs_i["implicit_gap_abs"].append(abs(implicit_gap))
                    logs_i["implicit_pred_between_0.2_0.8"].append(implicit_pred_between_02_08)
                    probs_tmp = probs_implicit_all_sds[(probs_implicit_all_sds!=-1).all(dim=-1)]
                    implicit_gap_sds = (((probs_tmp[:,1]>=0.5)*(probs_tmp[:,1]<=1)).float().mean() - ((probs_tmp[:,1]>=0)*(probs_tmp[:,1]<=0.5)).float().mean()).item()
                    logs_i["implicit_gap_abs_sds"].append(abs(implicit_gap_sds))

                ################################################
                # Step 2: generate dynamic targets (based on SDS probs)
                targets_all, uncertainty_all = generate_dynamic_targets(probs_implicit_all_sds, w_uncertainty=True)
                ...
                targets = targets_all[probs_implicit.shape[0]*(accelerator.local_process_index):probs_implicit.shape[0]*(accelerator.local_process_index+1)]
                uncertainty = uncertainty_all[probs_implicit.shape[0]*(accelerator.local_process_index):probs_implicit.shape[0]*(accelerator.local_process_index+1)]
                ...
                preds_implicit_ori_sds, probs_implicit_ori_sds, logits_implicit_ori_sds, _, _, _, _ = sds_logits_from_images(
                    images_ori,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                ...
                preds_implicit_ori, probs_implicit_ori, logits_implicit_ori, _ = clip_implicit_classifier(
                    images_ori,
                    clip_model=clip_model,
                    clip_processor=clip_processor,
                    text_features=implicit_text_features,
                ...
                if accelerator.is_main_process:
                    if step % args.train_plot_every_n_iter == 0:
                        ...
                        plot_in_grid(images_ori_all, save_to, preds_implicit=None, pred_class_probs_implicit=None)
            ...
            for j in range(N_backward):
                ...
                preds_implicit_ij, probs_implicit_ij, logits_implicit_ij, _, _, _, sds_realistic_ij = sds_logits_from_images(
                    images_ij,
                    tau=args.sds_tau,
                    t_min=args.sds_t_min,
                    t_max=args.sds_t_max,
                ...
                logits_implicit_ij = logits_implicit_ij.half()
                if idxs_valid.numel() > 0:
                    loss_fair_ij[idxs_valid] = CE_loss(logits_implicit_ij[idxs_valid], targets_ij[idxs_valid])
                ...
                preds_implicit_ori_sds_ij = preds_implicit_ori_sds[idxs_ij]
                dynamic_weights = gen_dynamic_weights_sds(
                    targets_ij,
                    preds_implicit_ori_sds_ij,
                    factor=args.factor1,
            ...
            if accelerator.is_main_process:
                ...
                for key in ["implicit_gap", "implicit_gap_abs", "implicit_gap_abs_sds","implicit_pred_between_0.2_0.8"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
